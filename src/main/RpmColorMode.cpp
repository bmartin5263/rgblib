...
auto SingleRpmColorMode::calculateColor(const RpmGaugeCalculations& calcs, const RpmEffect& scene) const -> rgb::Color {
  if (calcs.rpmLevelAchieved < calcs.yellowLevel) {
    return scene.greenColor;
  }
  else if (calcs.rpmLevelAchieved < calcs.redLevel) {
...
auto BlendedRpmColorMode::calculateColor(const RpmGaugeCalculations& calcs, const RpmEffect& scene) const -> rgb::Color {
...
auto PartitionedRpmColorMode::calculateColor(const RpmGaugeCalculations& calcs, const RpmEffect& scene) const -> rgb::Color {
